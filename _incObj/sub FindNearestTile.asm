...
FindNearestTile: ; Code adapted from Sonic 2
		move.w	d2,d0			; MJ: load Y position
		add.w	d0,d0			; MJ: multiply by 2
		andi.w	#$F00,d0		; MJ: get within (F00 pixels) in multiples of 80
		move.w	d3,d1			; MJ: load X position
		lsr.w	#3,d1			; MJ: shift to right side
		move.w	d1,d4
		lsr.w	#4,d1			; x_pos/128 = x_of_chunk - lsr.w #7, just like before, only now we are saving lsr.w #3 to d4 before finishing
		andi.w	#$7F,d1			; MJ: get within 7F
		add.w	d1,d0			; MJ: add calc'd Y to calc'd X. d0 is relevant chunk ID now		add.w	d1,d0			; MJ: add calc'd Y to calc'd X
		...
		andi.w	#$E,d4			; MJ: divide by 8
		add.w	d4,d1			; MJ: add to ror'd chunk ID
...
; (Original Sonic 1 calculated it every time instead of using a table)
word_1E5D0:
	dc.w	 0,  $80
	dc.w  $100, $180
	dc.w  $200, $280
