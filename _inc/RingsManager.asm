...
; ===========================================================================
RM_Index:	dc.w RM_Main-RM_Index ; 0
...
RM_Main:
		...
		move.w	(v_screenposx).w,d4		; Camera's X-position (left-most pixel displayed)
		subq.w	#8,d4
		bhi.s	@chkring
...
RM_Next:
	...
	@RM_31328C: ; -
		...
		cmpi.b	#$C,1(a1); is it destruction time yet? - 8 frame rings. Thank you Caverns4 REV C EDIT
...
Touch_Rings:
		...
		btst	#stsLightning,(v_status_secondary).w	; does the player have a lightning shield?
...
Touch_DestroyRing: ;+
		move.w	#$608,(a1)	; set frame and destruction timer - $608 instead of $604 for 8 frame rings - REV C EDIT
		bsr.w	CollectRing
...
Touch_Ring_AttractRing:
		...
		move.w	2(a3),obX(a1)		; Set x-position of object based on x-position in table
		move.w	4(a3),obY(a1)		; Do the same for the y-position
		;move.w	a4,$30(a1)
		move.w	#-1,(a3)
		rts
; ---------------------------------------------------------------------------
...
BuildRings_Loop:
		...
		move.b	(v_ani2_frame).w,d1	; use global frame
	@BRL_2: ;+
		add.w	d1,d1
		...
		addi.w	#$2533,d0	; add base art tile
