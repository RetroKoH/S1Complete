...
Pause_ChkReturn:
		...
		beq.s	Pause_ChkStart
		move.b	#id_MenuScreen,(v_gamemode).w	; set game mode to Time Attack Menu
		nop	
		bra.s	Pause_EndMusic
...
Pause_Reset: ; Iso Kilo; Concept by luluco
		...
		beq.s	Pause_Loop								; If so branch (This way you don't get 0 lives and then underflow)
		cmp.b	#id_Sonic_Death,(v_player+obRoutine).w
		bge.s	Pause_Loop								; prevent reseting when you die.
	...
	@timeattack:
		...
		move.b	#$80,(v_snddriver_ram+f_pausemusic).w 	; Unpause music
		clr.w	(f_pause).w
	@ret:
		rts 
